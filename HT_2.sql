...
--данных с указанием (при наличии) их книг, которые продаются магазинами
--издаются издательством.
Select a.FirstName+' '+a.LastName Author, b.NameBook Book, sh.NameShop Shop, sum(s.Quantity) SumQuantity
From Authors a 
join Books b on a.Id_Author=b.Id_Author
join Sales s on s.Id_Book=b.Id_Book
left join Shops sh on sh.Id_Shop=s.Id_Shop
Group by a.FirstName+' '+a.LastName , b.NameBook , sh.NameShop
...
--на 15%.
--Примечание! 75% запросов должны быть написаны в
--соответствии со стандартом SQL2